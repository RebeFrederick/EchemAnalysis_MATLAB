function DTA_master_struct(savelocation,summarieslocation)
%--------------------------------------------------------------------------
...
for k = 1:length(nameStructs) 
    ...
    switch testType
        case 'OCP'
            ...
            masterDTA.(wafer).(device).(electrode).(animal).(date).(testType) = struct('testID',test,'rawdata',rawdata,'calcdata',calcdata);
        case 'EIS'
        ...
        case 'CV'
            ...
            masterDTA.(wafer).(device).(electrode).(animal).(date).(testType).(scanrate) = struct('testID',test,'rawdata',rawdata,'calcdata',calcdata);
    end
    %
...
save(fullfile(summarieslocation,'masterDTA_Struct.mat'),'masterDTA');
%
%
% ------------------------------------------------------------------------
%                             END OF FILE
